...
for number in range(1, 6):
    ...
    for item_url in url_links:
        ...
        #
        price = ''.join(response.xpath('//div[@class="css-1texeil"]//text()'))
        print('price', price)

        item_number = response.xpath('//div[@class="css-ghc6s4"]/div/span//text()')
        item_numbers = ''.join([i_item_number + ' | ' for i_item_number in item_number ])
        ...
        prlces_txt = ''.join(response.xpath('//div[@class="css-1japfew"]/p//text()')).replace('\r', '').replace('\t', '').replace('\n', '')
        ...
        demographics_owners = ''.join([i_demographics_owner.replace('width:', '') + ' ｜ ' for i_demographics_owner in demographics_owner])
        print('demographics_owner', demographics_owners)



        with open('../data/raw/datascrap.csv', 'a+', encoding='utf-8-sig', newline='') as f:
            f = csv.writer(f)
            f.writerow([item_url]+[postcode]+[longitude]+[latitude]+[price]+[item_numbers]+[Description]+[domainsys]+[item_lists]+[year_tags]+[long_term]+
